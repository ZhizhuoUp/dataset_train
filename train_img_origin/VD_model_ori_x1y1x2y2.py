...
class ResNet(nn.Module):
    ...
    def loss(self, pred, target):
        ...
        ############### add another target while calculating the loss of cube ###############
        device = 'cuda:1'
        scaler = torch.tensor([[0.008, 0.008, 0, 0.008, 0.032],
                               [0, -0.008, -0.008, -0.008, 0.016]])
        scaler = scaler.to(device)

        target_2 = torch.clone(target)
        # print(target_2)
        target_2 = target_2 * (scaler[0] - scaler[1]) + scaler[1] # inverse_transform
        # target_2 = (target_2 - torch.min(target_2, 0)[0]) / (torch.max(target_2, 0)[0] - torch.min(target_2, 0)[0])
        for i in range(len(target_2)):
            if target_2[i, 4] < 0.017:
                ori = torch.atan2(target_2[i, 1] - target_2[i, 3], target_2[i, 0] - target_2[i, 2])
                # print('this is ori', ori)
                # print('this is target_2', target_2[i])
                grasp_point_2 = torch.tensor([[target_2[i, 0], target_2[i, 1]],
                                              [target_2[i, 2], target_2[i, 3]]], dtype=torch.float32)
                if ori < 0:
                    rotate = torch.tensor(3.1415926 / 2)
                    matrix = torch.tensor([[torch.cos(rotate), -torch.sin(rotate)],
                                           [torch.sin(rotate), torch.cos(rotate)]], dtype=torch.float32)
                    ...
                    pass
            # elif target_2[i, 4] > 0.017 and torch.abs(target_2[i, 1]) >= 0.0071:
            #     ori = torch.atan2(target_2[i, 1] - target_2[i, 3], target_2[i, 0] - target_2[i, 2])
            #     # print('this is ori', ori)
            #     # print('this is target_2', target_2[i])
            #     grasp_point_2 = torch.tensor([[target_2[i, 0], target_2[i, 1]],
            ...
            #         pass
            #     target_2[i, :4] = torch.reshape(torch.t((matrix.mm(torch.t(grasp_point_2)))), (-1,))
            #     print('this is target_2 after', target_2[i])
        # target_zzz = target.cpu().numpy()
        # target_zzz = scaler.inverse_transform(target_zzz)
        # target_zzz = target * (scaler[0] - scaler[1]) + scaler[1]
        # print('this is target before scaler', target_zzz)
        # print('this is target_2 before scaler', target_2)
        target_2 = (target_2 - scaler[1]) / (scaler[0] - scaler[1]) # transform
        # print('this is target', target)
        # print('this is target_2', target_2)
        # target_2 = target_2.to(device)

        value_true = (pred - target) ** 2

        for i in range(len(target)):
            # print('loss origin target', torch.mean((pred[i] - target[i]) ** 2))
            # print('loss improved target', torch.mean((pred[i] - target_2[i]) ** 2))
            if torch.mean((pred[i] - target[i]) ** 2) > torch.mean((pred[i] - target_2[i]) ** 2):
                # value.append(torch.mean((pred[i] - target_2[i]) ** 2))
                target[i] = torch.clone(target_2[i])
                # print('loss changed!')
            else:
                pass
                # value.append(torch.mean((pred[i] - target[i]) ** 2))
            # value.append(torch.min(torch.mean((pred[i] - target[i]) ** 2), torch.mean((pred[i] - target_2[i]) ** 2)))
        ...
        minmax = np.array([[0.008, 0.008, 0, 0.008, 0.032],
                [0, -0.008, -0.008, -0.008, 0.016]])
        scaler_iou = MinMaxScaler()
        scaler_iou.fit(minmax)
        # print(scaler.data_max_)
        # print(scaler.data_min_)

        value = (pred - target) ** 2

        # print('this is origin value', torch.mean(value_true))
        # print('this is modified value', torch.mean(value))

        IoU_loss = self.calculate_riou(pred, target, scaler_iou)
        # print('this is zzz_loss', IoU_loss)
        # print('this is value_loss', value)
        # width_loss = self.calculate_widthloss(pred[:, :4], target[:, :4])
        # print('this is width_loss', width_loss)

        result = value * (IoU_loss ** 3)
        # print(result)
